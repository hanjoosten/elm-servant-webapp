...
var $author$project$Main$update = F2(
	function (computer, mario) {
		var vx = $evancz$elm_playground$Playground$toX(computer.keyboard);
		var dt = 1.666;
		var vy = (!mario.y) ? (computer.keyboard.up ? 5 : 0) : (mario.vy - (dt / 8));
		var x = mario.x + (dt * vx);
		var y = mario.y + (dt * vy);
		return {
			dir: (!vx) ? mario.dir : ((vx < 0) ? 'left' : 'right'),
			vx: vx,
			vy: vy,
			x: x,
...
var $author$project$Main$toGif = function (mario) {
	return (mario.y > 0) ? ('https://elm-lang.org/images/mario/jump/' + (mario.dir + '.gif')) : ((!(!mario.vx)) ? ('https://elm-lang.org/images/mario/walk/' + (mario.dir + '.gif')) : ('https://elm-lang.org/images/mario/stand/' + (mario.dir + '.gif')));
};
		...
		return _List_fromArray(
			[
				...
				A3(
					...
					70,
					70,
					$author$project$Main$toGif(mario)))
			]);
	});
